...
const HeroSection = () => {
  ...
  const handleShopNow = () => {
    const productsSection = document.getElementById('new-arrivals-section');
    if (productsSection) {
  ...
  return (
    <div
      className='relative flex items-center bg-cover flex-start bg-center text-left h-screen w-full'
      style={{ backgroundImage: `url(${HeroImg})` }}
    >
      <div className='absolute top-0 right-0 bottom-0 left-0 bg-black/40'></div> {/* optional overlay */}
      <main className='px-10 lg:px-24 z-10'>
        <div className='text-left'>
          <h2 className='text-2xl text-white'>T-shirt / Tops</h2>
        </div>
